...
import higher_level_functions_aq_tactile as higher
...
source_dir      = os.path.join(home_dir, 'dataset-aq_tactile', 'sourcedata')
data_dir        = os.path.join(home_dir, 'dataset-aq_tactile', 'derivatives')
experiment_name = 'task-aq_tactile' # 3AFC Decision Task
...
# Participants
# -----------------------
ppns     = pd.read_csv(os.path.join(home_dir, 'combined_analysis', 'participants_aq_tactile.csv'))
subjects = ['sub-{}'.format(int(s)) for s in ppns['subject']]
flipped = np.array(ppns['flipped'])
# -----------------------
# AQ Scoring
# -----------------------
...
if pre_process:
    ...
    for s,subj in enumerate(subjects):
        edf = '{}_task-touch_prediction2_recording-eyetracking_physio'.format(subj)
        
        # which trial comes AFTER each break (some participants have 9 or 10 blocks depending on flip condition)
        if flipped[s]:
            break_trials = [22, 43, 64, 85, 106, 127, 148, 169, 190] 
        else:
            break_trials = [22, 43, 64, 85, 106, 127, 148, 169]
...
if higher_level:  
    ...
    ''' Note: the functions after this are using: task-aq_tactile_subjects.csv
    '''
    # higherLevel.code_stimuli()
    # higherLevel.calculate_actual_frequencies()
    ...
    ''' Average pupil response (time window) and conditions
    '''
