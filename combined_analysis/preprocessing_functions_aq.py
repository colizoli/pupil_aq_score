...
import glm_functions_aq as glm_functions # for nuisance regression
...
class pupilPreprocess(object):
    ...
    def __init__(self, subject, edf, experiment_name, project_directory, eye, break_trials, msgs, tolkens, sample_rate, tw_blinks, mph, mpd, threshold):
        ...
        if not os.path.isdir(os.path.join(project_directory, 'figures')):
            os.mkdir(os.path.join(os.path.join(project_directory, 'figures')))
    ...
    def extract_pupil(self,):
        ...
        Notes
        -----
        First column time stamp, 2nd column x position, 3rd column y position, 4th column is pupil data.
        Saves pupil time series and eye events in one numpy array, phases in another.
        """
        self.read_trials() # RUN FIRST
        # pupil_data = pd.read_csv(self.gazeOutputFileName, header=None, sep='\t',usecols=[0,1,2,3], names=['timestamp','x','y','pupil'])
        ...
        np.save(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio.npy'.format(self.subject, self.experiment_name)), np.array(TS))
        # save phase data separately, so can use same script for all experiments
        np.save(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio_phases.npy'.format(self.subject, self.experiment_name)), np.array(PHASES))
    ...
    def preprocess_pupil(self,):
        ...
        try:
            self.TS = pd.DataFrame(np.load(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio.npy'.format(self.subject, self.experiment_name))), columns=cols2)
        except:
            self.TS = pd.DataFrame(np.load(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio.npy'.format(self.subject, self.experiment_name))), columns=cols1)
        ...
        np.save(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio.npy'.format(self.subject, self.experiment_name)), np.array(self.TS))
    ...
    def extract_blocks(self,):
        ...
        """
        phases = pd.DataFrame(np.load(os.path.join(self.base_directory, '{}_{}_recording-eyetracking_physio_phases.npy'.format(self.subject, self.experiment_name))),columns=self.msgs[2:])
...
class trials(object):
    ...
    def __init__(self,subject, experiment_name, project_directory, sample_rate, phases, time_locked, pupil_step_lim, baseline_window):
        """Constructor method"""
    ...
    def event_related_subjects(self, pupil_dv):
        """Cut out time series of pupil data locked to time points of interest within the given kernel and mark blinks and saccades per trial.
        ...
        TS = pd.DataFrame(np.load(os.path.join(self.project_directory, '{}_{}_recording-eyetracking_physio.npy'.format(self.subject, self.experiment_name))), columns=cols)   
        ...
        phases = pd.DataFrame(np.load(os.path.join(self.project_directory, '{}_{}_recording-eyetracking_physio_phases.npy'.format(self.subject, self.experiment_name))))
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            if 'ESACC' in pupil_dv:
                SAVE_TRIALS.to_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_{}.csv'.format(self.subject, self.experiment_name, time_locked, pupil_dv)), float_format='%.16f')
            else:
                SAVE_TRIALS.to_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')
                SAVE_BLINKS.to_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_blinks.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')
                
            print('subject {}, {} events extracted'.format(self.subject,time_locked))
        print('success: event_related_subjects')
    ...
    def save_baselines(self):
        ...
        Notes
        -----
        Response-locked baselines are pre-cue, not pre-response!
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            if ('feed' in time_locked) or ('cue' in time_locked):
                
                pupil_step_lim = self.pupil_step_lim[t]

                P = pd.read_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked.csv'.format(self.subject, self.experiment_name, time_locked)))
                P.drop(['Unnamed: 0'],axis=1,inplace=True)
                P = np.array(P)
            
                baselines_file = os.path.join(self.project_directory, '{}_{}_{}_baselines.csv'.format(self.subject, self.experiment_name, time_locked))  # save baseline pupils
                SAVE_TRIALS = []
    ...
    def event_related_baseline_correction(self):
        ...
        Notes
        -----
        'resp_locked' corrected with 'cue' baseline
        """
        for t,time_locked in enumerate(self.time_locked):
            
            # for response_locked used cue-locked baseline
            if 'resp' in time_locked:
                time_locked_baselines = 'cue_locked'
            else:
            ...
            # baselines dataframe
            B = pd.read_csv(os.path.join(self.project_directory, '{}_{}_{}_baselines.csv'.format(self.subject, self.experiment_name, time_locked_baselines))) 
            ...
            P = pd.DataFrame(P)
            P.to_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_basecorr.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')

            print('subject {}, {} events baseline corrected'.format(self.subject, time_locked))
        print('success: event_related_baseline_correction')


    def event_related_saccades(self):
        """Marks each sample of the evoked response if a saccade was present and computes total samples per trial.
        """
        for t,time_locked in enumerate(self.time_locked):
            
            df_starts = pd.read_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_ESACC.csv'.format(self.subject, self.experiment_name, time_locked)), float_precision='high')
            df_ends = pd.read_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_ESACC_END.csv'.format(self.subject, self.experiment_name, time_locked)), float_precision='high')
            df_starts = df_starts.loc[:, ~df_starts.columns.str.contains('^Unnamed')] # remove all unnamed columns   
            df_ends = df_ends.loc[:, ~df_ends.columns.str.contains('^Unnamed')] # remove all unnamed columns   
            
            # mark all ends with -1
            df_ends[df_ends != 0] = -1
            
            # Convert the DataFrame to a NumPy array for efficient indexing and manipulation
            starts = np.array(df_starts)
            ends = df_ends.to_numpy()
            
            saccades = np.add(starts,ends)
            
            # loop trials
            for i in range(saccades.shape[0]):
                arr = saccades[i,:] # current trial/row
                # Find the indices of the first '1' and first '-1'
                try:
                    first_one_index = np.where(arr == 1)[0][0]
                except IndexError:
                    first_one_index = None

                try:
                    first_minus_one_index = np.where(arr == -1)[0][0]
                except IndexError:
                    first_minus_one_index = None

                # Check for the normal case: '1' is found before '-1'
                if first_one_index is not None and first_minus_one_index is not None and first_one_index < first_minus_one_index:
                    arr[first_one_index + 1:first_minus_one_index] = 100
                # Check for the edge case: '-1' is found, but no '1' precedes it
                elif first_minus_one_index is not None and (first_one_index is None or first_one_index > first_minus_one_index):
                    arr[:first_minus_one_index] = 100
                
                # fill trial/row back in with new data
                saccades[i,:] = arr
                
            # mark all samples with 1
            saccades[saccades != 0] = 1 # uncomment here to check visually in output files if algorithm is working!
            saccades = pd.DataFrame(saccades)
            saccades.to_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_saccades.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')
            
            # sum samples per trial and save
            saccades = pd.DataFrame(np.sum(saccades, axis=1), columns=['saccade_nsamples'])
            saccades.to_csv(os.path.join(self.project_directory, '{}_{}_{}_trials_saccades.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')
            print('subject {}, {} event_related_saccades'.format(self.subject, time_locked))
        print('success: event_related_saccades')


    def event_related_blinks(self):
        """Marks each sample of the evoked response if a saccade was present and computes total samples per trial.
        """
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            this_blinks = pd.read_csv(os.path.join(self.project_directory, '{}_{}_{}_evoked_blinks.csv'.format(self.subject, self.experiment_name, time_locked)))
            ...
            # sum samples per trial and save
            this_df.to_csv(os.path.join(self.project_directory, '{}_{}_{}_trials_blinks.csv'.format(self.subject, self.experiment_name, time_locked)), float_format='%.16f')
            print('subject {}, {} event_related_blinks'.format(self.subject, time_locked))
        print('success: event_related_blinks')