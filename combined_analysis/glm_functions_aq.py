...
class GeneralLinearModel(object):
	...
	def execute(self):
		...
		self.design_matrix = np.asmatrix(self.design_matrix).T
		
		# GLM:
		self.betas = np.array(((self.design_matrix.T * self.design_matrix).I * self.design_matrix.T) * np.asmatrix(self.working_data_array).T).ravel()
	...
	def add_stick_regressor(self, regressor):
		...
		for event in regressor:
			...
			regressor_values[int(start_time)] = event[2]
