...
class higherLevel(object):
    ...
    def higherlevel_get_phasics(self,):
        ...
        """
        for s,subj in enumerate(self.subjects):
            this_log = os.path.join(self.project_directory, subj, '{}_task-letter_color_visual_decision_beh.csv'.format(subj)) # derivatives folder
    ...
    def create_subjects_dataframe(self, exclude_interp):
        ...
        for s,subj in enumerate(self.subjects):
            ...
            this_data = pd.read_csv(os.path.join(self.project_directory, subj, '{}_task-letter_color_visual_decision_beh.csv'.format(subj)), float_precision='high')
            this_data = this_data.loc[:, ~this_data.columns.str.contains('^Unnamed')] # remove all unnamed columns
            this_data = this_data.loc[:, ~this_data.columns.str.contains('blinks')] # remove all old blink columns
            this_data = pd.read_csv(os.path.join(self.project_directory, subj, '{}_task-letter_color_visual_decision_beh.csv'.format(subj)), float_precision='high')
        ...
        DF.sort_values(by=['subject', 'trial_num'], ascending=[True, True])
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            for c,cond in enumerate(csv_names):
                ...
                for s,subj in enumerate(self.subjects):
                    ...
                    evoked_cols = np.char.mod('%d', np.arange(SPUPIL.shape[-1]))
                    SPUPIL.columns = evoked_cols
        ...
        for t,time_locked in enumerate(self.time_locked):
            ...
            for cond in ['subject', 'correct', 'frequency', 'correct-frequency']:
                ...
                if cond == 'correct':
                    ...
                    this_pupil['correct'] = this_pupil['correct'].astype(int)
                    ...
                    try: # conservative analysis, no correct-frequency condition
                        term1 = np.subtract(easy_error, easy_correct)
                        term2 = np.subtract(hard_error, hard_correct)
    ...
    def plot_evoked_correlation_AQ(self):
        """Plot partial correlation between pupil response and AQ scores.
    ...
    def regression_pupil_AQ(self, ):
        ...
        for cond in ['frequency']:
            ...
            if cond == 'frequency': 
                ...
                # 20%-80%
                merged['pupil'] = np.array(merged['20']-merged['80'])
                ...
                for i,dv in enumerate(dvs):
                    ...
                    if (dv =='correct' and cond == 'correct') or (dv =='correct' and cond == 'correct-frequency'):
                        ...
                        if cond == 'low_frequency' or cond == 'high_frequency':
                            P = pd.read_csv(os.path.join(self.jasp_folder,'{}_{}_{}_rmanova.csv'.format(self.exp, 'frequency', dv)), float_precision = 'high')
                        else:
    ...
    def plot_AQ_histogram(self, ):
        ...
        """
        y_min = 0 # lowest score per subscale
        y_max = 20 # across both datasets!
    ...
    def plot_behav_blocks(self):
        ...
        for dv in dvs:
            ...
            df.columns = [int(float(c)) if str(c).replace('.', '', 1).isdigit() else c for c in df.columns] # 1.00000000 to 1
            
            df = df[blocks] # get only block data
            means = df.mean()
    ...
    def regression_pupil_AQ_blocks(self, ):
        ...
        for block in [1,2,3,4]:
            ...
            if cond == 'block-frequency': 
                # 20%-80%
                merged['pupil'] = np.array(merged['(20, {}.0)'.format(block)]-merged['(80, {}.0)'.format(block)]) # blocks are different columns
    ...
    def information_theory_estimates(self, flat_prior=False):
        ...
        if flat_prior:
            ...
            priors = pd.read_csv(os.path.join(self.dataframe_folder,'{}_subjects_priors.csv'.format('task-aq_visual_training')), float_precision='%.16f')
